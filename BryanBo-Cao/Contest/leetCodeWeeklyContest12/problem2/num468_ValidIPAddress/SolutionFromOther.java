...
public class SolutionFromOther {
    public String validIPAddress(String IP) {
        if (IP.indexOf(".") > 0) {
            if (parse4(IP)) return "IPv4";
        } else if (IP.indexOf(":") > 0) {
            if (parse6(IP)) return "IPv6";
        }
        return "Neither";
    }

    private boolean parse4(String ip) {
    ...
    private boolean parse6(String ip) {
        String[] parts = ip.split(":");
        ...
        for (int i = 0; i < 8; i++) {
            ...
            for (int j = 0; j < part.length(); j++) {
                if (Character.isAlphabetic(part.charAt(j))) {
